...
async function loadPhoto(){
  ...
  if(!path){$('#socio-photo').hidden=true;$('#socio-photo-placeholder').hidden=false;return}
  const {data}=await supabase.storage.from('fotografias-socios').createSignedUrl(path,3600);
  if(data?.signedUrl){$('#socio-photo').src=data.signedUrl;$('#socio-photo').hidden=false;$('#socio-photo-placeholder').hidden=true}
}

...
async function loadQuotas() {
    ...
    try {
        ...
        const isUnpaidQuota = (q) => {
            const status = String(q.estado || 'pendente').trim().toLowerCase();
            return q.pago !== true && !['pago', 'paga', 'isento', 'anulado'].includes(status);
        };

        /*
         * A quota anual não paga conta para a dívida do sócio. Esta regra
         * mantém a área do sócio alinhada com a administração: o sócio 132,
         * por exemplo, tem três registos anuais não pagos e deve ver os três.
         */
        const atrasadas = quotas.filter(isUnpaidQuota);
        const pagas = quotas.filter(q => !isUnpaidQuota(q) && quotaStatusLabel(q.estado) === 'Paga');
        const pendentes = quotas.filter(q => isUnpaidQuota(q) && quotaStatusLabel(q.estado) === 'Pendente');

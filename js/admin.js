(() => {
  ...
  function getSharedClient() {
    return window.NAF_GET_SHARED_SUPABASE?.() || window.__NAF_SUPABASE || window.supabaseClient || null;
  }
  ...
  async function loadMembers() {
    ...
    if (ids.length) {
      const [pointsResult, quotasResult] = await Promise.all([
      ...
      if (!pointsResult.error) {
        for (const p of pointsResult.data || []) {
          const k = String(p.socio_id);
          totals.set(k, (totals.get(k) || 0) + Number(p.pontos || 0));
        }
      } else console.warn('[ADMIN] Não foi possível carregar os pontos:', pointsResult.error);

      const overdue = new Map();
      if (!quotasResult.error) {
        const current = new Date();
        const currentMonth = new Date(current.getFullYear(), current.getMonth(), 1);
        for (const q of quotasResult.data || []) {
          if (!q.mes) continue;
          ...
          if (unpaid && d < currentMonth) {
            const k = String(q.socio_id);
            const x = overdue.get(k) || { months: 0, value: 0 };
            x.months += 1;
            x.value += Number(q.valor || 0);
            overdue.set(k, x);
          }
        }
      } else console.warn('[ADMIN] Não foi possível carregar as quotas:', quotasResult.error);
  ...
  async function sendPointsMail(member, action, extra) {
    ...
    const added = action === 'pontos_adicionados';
    const points = Number(extra?.pontos_adicionados ?? extra?.pontos_retirados ?? 0);
    const activity = extra?.atividade || 'Fun&Learn';
    const reason = extra?.descricao || extra?.motivo || '';
    const subject = added ? 'Pontos Fun&Learn adicionados' : 'Pontos Fun&Learn retirados';
    const text = added
      ? `Olá ${member.nome || 'sócio'},\n\nForam adicionados ${points} ponto(s) à sua conta de Fun&Learn.\nAtividade: ${activity}.\nMotivo: ${reason}\n\nNúcleo de Árbitros de Futebol Marques Bom`
      : `Olá ${member.nome || 'sócio'},\n\nForam retirados ${points} ponto(s) da sua conta de Fun&Learn.\nMotivo: ${reason}\n\nNúcleo de Árbitros de Futebol Marques Bom`;
    await invokeEdge(getCfg().EMAIL_FUNCTION || 'admin-mail', {
      to: member.email,
      subject,
      text
    });
  ...
  async function sendOverdueSelected(){const ids=selectedOverdueIds();if(!ids.length)throw new Error('Seleciona pelo menos um sócio com quotas em atraso e email.');const subject=$('quota-subject')?.value?.trim()||'Quotas em atraso';const message=$('quota-message')?.value?.trim()||'';if(!message)throw new Error('Escreve a mensagem do aviso de quotas.');let sent=0;const failures=[];for(const id of ids){const member=state.members.find(m=>String(m.id)===id);if(!member?.email)continue;try{await invokeEdge(getCfg().EMAIL_FUNCTION || 'send-email',{to:member.email,subject,text:message.replaceAll('{NOME}',member.nome||'sócio')});sent++;}catch(error){failures.push(`${member.nome}: ${error.message||error}`);}}if(failures.length)throw new Error(`${sent} aviso(s) enviado(s); ${failures.length} falharam. ${failures.slice(0,3).join(' | ')}`);show(`Avisos processados: ${sent} enviado(s).`);}
  ...
  function bind(){
    ...
    document.querySelectorAll('.admin-tab').forEach(btn=>{if(btn.dataset.consolidatedBound==='1')return;btn.dataset.consolidatedBound='1';btn.addEventListener('click',()=>{document.querySelectorAll('.admin-tab').forEach(x=>{x.classList.toggle('active',x===btn);x.setAttribute('aria-selected',String(x===btn));});document.querySelectorAll('.admin-tab-panel').forEach(x=>{const active=x.id===`panel-${btn.dataset.panel}`;x.classList.toggle('active',active);x.hidden=!active;});});});
  }
  async function checkAdminSession() {
    const client = getSharedClient();
    if (!client) throw new Error('Biblioteca/cliente Supabase não disponível.');
  ...
  async function handleAdminLogin(event) {
    ...
    const client = getSharedClient();
    if (!client) return fail(new Error('Ligação ao Supabase indisponível.'));
    const email = $('admin-login-email')?.value.trim();
    const password = $('admin-login-password')?.value || '';
    if (!email || !password) return fail(new Error('Indica o email e a palavra-passe.'));
  ...
  async function initialiseAdmin() {
    try {
      ...
      const warning = $('admin-login-warning');
      if (!allowed) {
        $('admin-app')?.setAttribute('hidden', '');
        warning?.removeAttribute('hidden');
        return false;
      }
      warning?.setAttribute('hidden', '');
      $('admin-app')?.removeAttribute('hidden');
      bind();
      await loadMembers();
      return true;
    } catch (error) {

-- Migração necessária para a integração final do painel administrativo.
-- As tabelas/policies são idempotentes.
...
create policy acoes_admin_all on public.acoes for all to authenticated
using (exists(select 1 from public.socios s where s.user_id=auth.uid() and s.ativo=true and s.is_admin=true))
with check (exists(select 1 from public.socios s where s.user_id=auth.uid() and s.ativo=true and s.is_admin=true));

drop policy if exists acoes_socios_active_select on public.acoes;
create policy acoes_socios_active_select on public.acoes for select to authenticated
using (ativa=true and inscricoes_abertas=true);

drop policy if exists acoes_inscricoes_admin_all on public.acoes_inscricoes;
create policy acoes_inscricoes_admin_all on public.acoes_inscricoes for all to authenticated
using (exists(select 1 from public.socios s where s.user_id=auth.uid() and s.ativo=true and s.is_admin=true))
with check (exists(select 1 from public.socios s where s.user_id=auth.uid() and s.ativo=true and s.is_admin=true));

drop policy if exists acoes_inscricoes_own_select on public.acoes_inscricoes;
create policy acoes_inscricoes_own_select on public.acoes_inscricoes for select to authenticated
using (exists(select 1 from public.socios s where s.id=socio_id and s.user_id=auth.uid() and s.ativo=true));

drop policy if exists acoes_inscricoes_own_insert on public.acoes_inscricoes;
create policy acoes_inscricoes_own_insert on public.acoes_inscricoes for insert to authenticated
with check (
 exists(select 1 from public.socios s where s.id=socio_id and s.user_id=auth.uid() and s.ativo=true)
 and exists(select 1 from public.acoes a where a.id=acao_id and a.ativa=true and a.inscricoes_abertas=true
 and (a.prazo_inscricao is null or a.prazo_inscricao>=now())
...
create policy acoes_inscricoes_own_update on public.acoes_inscricoes for update to authenticated
using (exists(select 1 from public.socios s where s.id=socio_id and s.user_id=auth.uid() and s.ativo=true))
with check (exists(select 1 from public.socios s where s.id=socio_id and s.user_id=auth.uid() and s.ativo=true));

/* NAF Marques Bom — pagamento manual de quotas.
   Idempotente. Não apaga quotas nem recibos existentes. */

create or replace function public.registar_pagamento_manual(p_socio_id uuid, p_valor numeric, p_metodo text)
...
begin
  ...
  v_needed := round(p_valor/12.00)::integer;

  /* Se não existirem quotas suficientes, cria apenas as anuais em falta. */
  if (select count(*) from public.quotas q
      where q.socio_id=p_socio_id
        ...
        and round(q.valor,2)=12.00) < v_needed then
    v_year := extract(year from current_date)::integer;
    while (select count(*) from public.quotas q
           where q.socio_id=p_socio_id
             and coalesce(q.pago,false)=false
             and lower(coalesce(q.estado,'pendente')) not in ('pago','paga','isento','anulado')
             and round(q.valor,2)=12.00) < v_needed loop
  ...
  values(p_socio_id,v_sum,p_metodo,v_items,auth.uid()) returning * into v_receipt;
